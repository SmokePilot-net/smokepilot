...
def add_slave():
    ...
    try:
        ...
        flash(f"Slave '{name}' created. API key (copy now — shown once): {raw_key}", "success")
    except Exception as e:
        flash(f"Error: {e}", "error")
    return redirect(url_for("slaves_page"))
...
def serve_agent_installer(slave_id):
    ...
    script = f"""#!/bin/bash
set -e
# SmokePing Manager Agent — auto-installer for slave: {slave['display_name'] or slave['name']}
# Generated by {public_url}

if [ "$EUID" -ne 0 ]; then
    echo "Please run as root: curl -s {public_url}/slaves/install/{slave_id} | sudo bash"
    exit 1
fi

INSTALL_DIR="/opt/smokeping-agent"
echo "=== Installing SmokePing Manager Agent ==="
echo "  Slave: {slave['display_name'] or slave['name']}"
echo "  Master: {public_url}"
...
# Config
cat > /etc/smokeping-agent.env << 'ENV_EOF'
SPM_MASTER_URL={public_url}
SPM_SLAVE_KEY=REPLACE_WITH_API_KEY
...
echo ""
echo "  IMPORTANT: Edit /etc/smokeping-agent.env and set SPM_SLAVE_KEY"
echo "  (The API key was shown when you created this slave in the UI)"
...
ExecStart=/usr/bin/python3 /opt/smokeping-agent/smokeping_agent.py
Restart=on-failure
RestartSec=10
...
echo ""
echo "=== Done ==="
echo "  1. Set the API key: sudo nano /etc/smokeping-agent.env"
echo "  2. Start: sudo systemctl start smokeping-agent"
echo "  3. Check: sudo systemctl status smokeping-agent"
"""
