...
from auth import (
    auth_required, hash_password, check_password, generate_api_token as gen_api_token,
...
# --- Manage (target administration, admin only) ---
...
@auth_required()
def set_style():
    style = request.form.get("style", "light")
    # Community styles only (compact SVG is Pro)
...
def add_user():
    ...
    # Community: admin and viewer only (operator is Pro)
...
def create_token():
    ...
    # Community: limit to 1 token per user
    existing = get_api_tokens(g.current_user["id"])
    if len(existing) >= 1:
        flash("Community edition allows 1 API token per user. Upgrade to Pro for unlimited tokens.", "error")
        return redirect(url_for("settings"))

    raw_token, token_hash, token_prefix = gen_api_token()
