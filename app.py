...
def add_slave():
    ...
    try:
        ...
        # Show setup page with embedded API key (only time the key is visible)
        return redirect(url_for("slave_setup", slave_id=slave_id, key=raw_key))
    except Exception as e:
        flash(f"Error: {e}", "error")
    return redirect(url_for("slaves_page"))
...
def slave_setup(slave_id):
    ...
    public_url = PUBLIC_URL or request.host_url.rstrip("/")
    return render_template("slave_setup.html", slave=slave, api_key=api_key, public_url=public_url)
